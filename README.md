...
- またこれは学園祭の展示用に作られたもので、完成度は高くないことをご了承ください。  
### 開発者情報  
...
| ファイル | TODO |
|---|---|
| `DialogSamples.java` | ボス専用立ち絵への差し替え(現在は3ステージ分のみ) |
...
- ステージ別のボス会話シナリオ(全6面・プリ/ポスト個別)  
- キャラクター別のエンディング分岐(Uhyoman / Uhyowoman)  
- BossのHP閾値による弾幕の段階変化(残量75/50/25%でwayや本数・追い打ちが激化)  
- ステージ別テーマ色の多層パララックス背景(BackgroundLayer × ParallaxBackground)  
- 設定のセーブ/ロード(`~/.shooting_uhyo/options.properties`・難易度/音量/ヒットボックス表示)  
- ゲームオーバーからのコンティニュー(Zキー・1プレイ最大3回)  
- ボス顔アイコンの自動センタリング(透過余白をトリミングし正方形中央へ再配置)  
